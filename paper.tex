...
all nodes of a given multicore machine. First, we discuss what
kind of overlay networks make sense on multicores.

As a first step, we evaluate tree based overlay networks. Other
potentially interesting topologies are rings.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Tree overlay}
...
\stefan{Just a random collection of ideas}

%-------------------------------------------------
\subsubsection{Load balancing in a tree}
...
can also have different sizes (e.g.\ a smaller one for read operations
and larger ones for writes).
...
\stefan{This section feels out of place. Rewrite it and put it
  somewhere else}
...
Multikernel operating system. It is, however, centralized. The idea is
to make Octopus a decentralized application.
...
synchronization primitives even makes sense. As we want to show
efficiency, we probably rather want to implement new synchronization
